...
uint8_t motor_AllGoHome(void) /*要小改一下*/
{
	uint32_t tick = 0;
#if dbg
	goHomeFlag_dbg = 0;
	...
	HAL_Delay(100);
	// /*----------------------------------------------------------------------------------*/
	motorGoPosition(3, 1000, 100, 728);
	motorGoPosition(4, 1000, 100, -865);
	while (motor_statuses[2].is_reach == 0 || motor_statuses[3].is_reach == 0)
	{
		HAL_Delay(100);
		tick++;
...
void PutDown_posInit(uint32_t PutDown_offset, uint32_t heigh) // 料盘放置位置初始化
{
	...
	PutDown_position[0] = (Max_Length * 16384 / lead_screw); // 顶层放料带点
}
...
void MoveFirstGMotors(uint8_t state, uint16_t speed, uint8_t acc)
{
	if (state == 0) // 钩子张开
	{
		motorGoPosition(1, speed, acc, -232775); // 88 2422
		HAL_Delay(2);
		motorGoPosition(2, speed, acc, -235000); // 90 2433
	}
	else if (state == 1)
	{
...
// #endif
	...
	{
#if dbg
		motorGoPosition(3, 500, 50, 84468);	 // 458
		motorGoPosition(4, 500, 50, -84423); // 458
#else
		motorGoPosition(3, 500, 50, 84468);	 // 458
		motorGoPosition(4, 500, 50, -84423); // 458
#endif
	...
	{
#if dbg
		motorGoPosition(3, speed, acc, 728);
		motorGoPosition(4, speed, acc, -865);
#else
		motorGoPosition(3, speed, acc, 728);
		motorGoPosition(4, speed, acc, -865);
#endif
...
uint8_t GetMaterial(uint8_t num) // 去取num盘料
{
	...
	while (motor_statuses[4].is_reach == 0)
	{
		MoveFifthMotor(300, 30, tray_position[num]);
	...
	motor_statuses[4].is_reach = 0;
#if dbg
	heigh_dbg = tray_position[num] / 16384.0 * lead_screw;
#endif
	...
	while (motor_statuses[2].is_reach == 0 || motor_statuses[3].is_reach == 0)
	{
		// MoveSecondGMotors(1, 500, 50, num);
		motorGoPosition(3, 300, 50, 728 - 182 * 2 + (tray_num - 1 - num) * (182 * 2) / (tray_num - 2));	   // 0
		motorGoPosition(4, 300, 50, -(865 - 182 * 2 + (tray_num - 1 - num) * (182 * 2) / (tray_num - 2))); // 0
	...
	while (motor_statuses[4].is_reach == 0)
	{
		MoveFifthMotor(300, 30, tray_position[0]);
		while (motor_statuses[4].is_reach != 1)
		{
	...
	motor_statuses[4].is_reach = 0;
#if dbg
	heigh_dbg = tray_position[0] / 16384.0 * lead_screw;
#endif
	return 0;
}

uint8_t GetMaterialBack(uint8_t num) // 去取num盘料
{
	uint32_t tick = 0;
	uint8_t timeoutFlag = 0; // 超时标志
	while (stop_state != 0)
	...
	while (motor_statuses[2].is_reach == 0 || motor_statuses[3].is_reach == 0)
	{
		// MoveSecondGMotors(0, 500, 50, num);
		motorGoPosition(3, 300, 50, 83922 + (tray_num - 1 - num) * (182 * 5) / (tray_num - 2));		// 455
		motorGoPosition(4, 300, 50, -(83877 + (tray_num - 1 - num) * (182 * 5) / (tray_num - 2))); // 455
		while (motor_statuses[2].is_reach != 1 || motor_statuses[3].is_reach != 1)
		{
	...
	{
		MoveFirstGMotors(0, 1000, 100);
		motorGoPosition(1, 1000, 100, -227931); // 86 2422
		// HAL_Delay(2);
		motorGoPosition(2, 1000, 100, -227701); // 87 2433
		while (motor_statuses[0].is_reach != 1 || motor_statuses[1].is_reach != 1)
		{
	...
	{
		// MoveSecondGMotors(1, 500, 50, num);
		motorGoPosition(3, 300, 50, 728 - (tray_num - 1 - num) * (182 * 2) / (tray_num - 2));	   // 0
		motorGoPosition(4, 300, 50, -(865 -  (tray_num - 1 - num) * (182 * 2) / (tray_num - 2))); // 0
		while (motor_statuses[2].is_reach != 1 || motor_statuses[3].is_reach != 1)
		{
...
uint8_t PutDownMaterial(uint8_t num) // 放第num盘料
{
	...
	while (motor_statuses[4].is_reach == 0)
	{
		MoveFifthMotor(300, 30, PutDown_position[0]);
		while (motor_statuses[4].is_reach != 1)
		{
			tick += 1;
			if (tick >= 1000)
...
#if dbg
	heigh_dbg = PutDown_position[0] / 16384.0 * lead_screw;
#endif
	motor_statuses[4].is_reach = 0;
	...
	while (motor_statuses[2].is_reach == 0 || motor_statuses[3].is_reach == 0)
	{
		motorGoPosition(3, 300, 50, 84832 - (tray_num - 1 - num) * (182 * 5) / (tray_num - 2));		// 460
		motorGoPosition(4, 300, 50, -(84787 - (tray_num - 1 - num) * (182 * 5) / (tray_num - 2))); // 460
	...
	while (motor_statuses[4].is_reach == 0)
	{
		MoveFifthMotor(300, 30, PutDown_position[num]);
		while (motor_statuses[4].is_reach != 1)
		{
...
#if dbg
	heigh_dbg = PutDown_position[num] / 16384.0 * lead_screw;
#endif
	motor_statuses[4].is_reach = 0;
...
uint8_t PutDownMaterialBack(uint8_t num) // 放第num盘料
{
	uint32_t tick = 0;
	uint8_t timeoutFlag = 0;
	/*---------------------------整体降到放料点-------------------------*/
